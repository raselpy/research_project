...
def assign_holdout_split(case_ids: list[str], holdout_ratio: float, fold_seed: int) -> dict[str, str]:
    """Reserves holdout_ratio of case_ids as a genuine holdout set —
    excluded from k-fold assignment entirely, so NONE of the 5 CV
    models ever see it during training. Returns {case_id: "holdout" |
    "train_pool"}.

    GAP FILLED HERE: Phase 8's ensembling (ensemble_predict) needs to be
    evaluated on cases no fold trained on — matching the paper's own
    practice (Section 3.2: "we use the five models obtained from the
    cross-validation... as an ensemble to predict the validation set",
    a genuinely separate set from any fold's training data). The
    original Phase 8 implementation only k-fold-split the *entire*
    dataset, so every case belonged to exactly one fold's own
    validation set — meaning an "ensemble" evaluated against those
    cases would have 4 of 5 models having already trained on each one.
    This reserves a true holdout slice first, via
    cfg.dataset.split_ratios' third value (already documented in
    dataset_schema.py's own comment as intended for exactly this, but
    never wired up until now).
    """
    rng = np.random.default_rng(fold_seed)
    shuffled = rng.permutation(case_ids).tolist()
    n_holdout = max(round(len(case_ids) * holdout_ratio), 1) if case_ids else 0
    holdout_ids = set(shuffled[:n_holdout])
    return {case_id: ("holdout" if case_id in holdout_ids else "train_pool") for case_id in case_ids}
...
def main(
    ...
    all_case_ids = [row["case_id"] for row in manifest_rows]
    split_of_case = assign_holdout_split(all_case_ids, holdout_ratio, fold_seed)

    train_pool_ids = [c for c in all_case_ids if split_of_case[c] == "train_pool"]
    fold_of_case = assign_folds(train_pool_ids, num_folds, fold_seed)

    for row in manifest_rows:
        row["split"] = split_of_case[row["case_id"]]
        # holdout cases have no fold membership — never trained on by any fold's model.
        row["fold"] = fold_of_case.get(row["case_id"], -1)
    ...
    logger.info(
        ...
        f"({num_folds}-fold assignment on the train_pool, seed={fold_seed}, "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Fraction of cases reserved as a genuine holdout set, excluded from all k-fold training (used for ensemble evaluation).",
    )
    args = parser.parse_args()
    main(args.path, num_folds=args.num_folds, fold_seed=args.fold_seed, holdout_ratio=args.holdout_ratio)
